...
const ghostLinkClass =
  "text-sm font-medium text-[#8B9E6B] transition-colors hover:text-[#7A8A5A] hover:underline";
const dashboardCardClass =
  "flex h-full min-h-0 flex-col rounded-2xl border border-[#D4C9A8] bg-white p-5 shadow-sm md:p-6";
...
export function PortalSystemsDashboard({
      ...
      >
        <div className="flex items-center gap-3">
          <div className="portal-hero-icon-fade-in shrink-0" aria-hidden>
            ...
            >
              {heroAllOk ? (
                <Check className="h-6 w-6 text-[#6A8F5A]" strokeWidth={2.5} />
              ) : (
                <AlertTriangle
          ...
          <div className="min-w-0 flex-1 text-left">
            <h1
              className={`text-lg font-semibold leading-snug tracking-tight md:text-xl ${
                heroAllOk ? "text-[#2C3020]" : "portal-hero-warning-shimmer"
              }`}
            >
              {heroAllOk
                ? "Alt fungerer"
            ...
            <p className="mt-0.5 text-[11px] text-[#8C8A78]">Senest tjekket {latestCheckSubtext}</p>
          </div>
        </div>
  ...
  const renderSystemRow = (row: SystemRowModel, tabKey: string, index: number, animate: boolean) => (
    ...
    >
      <button
        type="button"
        onClick={() => openDetail(row)}
        className={`group/dash-row flex w-full items-center gap-3 px-4 py-4 text-left transition-colors duration-150 hover:bg-[#EEF2E6] ${rowAccentClass(row.status)}`}
      >
        <span
          ...
          aria-hidden
        />
        <span className="min-w-0 flex-1 text-sm font-semibold text-[#2C3020]">{row.friendly}</span>
        <span className="hidden shrink-0 text-xs font-medium text-[#5C5A48] sm:block">
        ...
        {row.status === "afventer" ? (
          <Link
            ...
            className="shrink-0 text-[11px] text-[#8C8A78] hover:text-[#8B9E6B]"
          >
            Opsæt →
          </Link>
        ) : (
          <span className="hidden shrink-0 text-[10px] text-[#8C8A78] md:inline">{row.checked}</span>
        )}
      </button>
    </li>
  );
  ...
  if (loading) {
    return <p className="text-sm text-[#8C8A78]">Indlæser overblik...</p>;
  }

  return (
    <div className="relative flex h-full min-h-0 w-full max-w-none flex-col md:overflow-hidden">
      {preview ? (
        <p className="mb-4 text-center text-xs text-[#8C8A78]">
          Forhåndsvisning af kundens portal-overblik
        </p>
      ) : null}

      {!hasSystems ? (
        <div className="rounded-2xl border border-[#D4C9A8] bg-white px-6 py-10 text-center text-sm text-[#5C5A48] shadow-sm">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 items-stretch gap-4 md:gap-5 lg:grid-cols-3 lg:grid-rows-[minmax(0,1fr)_auto]">
          ...
          >
            <h2 className="text-sm font-semibold uppercase tracking-wide text-[#8C8A78]">
              Systemstatus
            </h2>
              ...
              {!monitoringLoading ? (
                <p className="text-sm text-[#5C5A48]">
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full bg-[#6A8F5A]" aria-hidden />
                    <span className="font-semibold tabular-nums text-[#2C3020]">{displayOk}</span> OK
                  </span>
                  <span className="mx-2 text-[#D4C9A8]">·</span>
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full bg-[#C4A84F]" aria-hidden />
                    <span className="font-semibold tabular-nums text-[#2C3020]">{displayAdvarsel}</span> Advarsel
                  </span>
                  <span className="mx-2 text-[#D4C9A8]">·</span>
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full bg-[#94A3B8]" aria-hidden />
                    <span className="font-semibold tabular-nums text-[#2C3020]">{displayAfventer}</span> Afventer
                  </span>
                </p>
              ) : null}
              <div
                className="-mx-2 flex min-h-[200px] flex-1 flex-col overflow-hidden rounded-xl border border-[#D4C9A8] md:min-h-0"
                onMouseEnter={() => setTabsHovered(true)}
                onMouseLeave={() => setTabsHovered(false)}
              >
                <div ref={tabNavRef} data-tour="dashboard-system-tabs" className="relative border-b border-[#D4C9A8]">
                      ...
                      return (
                        <button
                          ...
                          className={`relative shrink-0 overflow-hidden px-4 py-3 text-sm font-medium transition-colors duration-150 ${
                            isActive ? "text-[#2C3020]" : "text-[#8C8A78] hover:text-[#5C5A48]"
                          }`}
                        >
                          {g.shortLabel}
                  ...
                  <span
                    className="pointer-events-none absolute bottom-0 left-0 h-[2px] overflow-hidden bg-[#EEF2E6]/80 transition-[transform,width] duration-300 ease-out"
                    style={{
                  ...
                  >
                    {!monitoringLoading && !preview && tabsWithSystems.length > 1 ? (
                      <span
                        key={tabProgressKey}
                        className={`portal-tab-progress block h-full bg-[#8B9E6B] ${
                ...
                <div className="min-h-0 flex-1 overflow-hidden" aria-busy={monitoringLoading}>
                  {tabsWithSystems.map((g) => {
                    ...
                    return (
                      <ul
                        key={tabKey}
                        className={`divide-y divide-[#E8E2D0] ${isActive ? "block" : "hidden"}`}
          ...
          {!preview ? (
            <article data-tour="dashboard-active-tickets" className={`${dashboardCardClass} min-h-0`}>
              <div className="flex items-center gap-2">
                <h2 className="text-sm font-semibold text-[#2C3020]">Aktive sager</h2>
                {!ticketsLoading ? (
                  <span className="rounded-full bg-[#EEF2E6] px-2 py-0.5 text-xs font-semibold tabular-nums text-[#2C3020]">
                    {activeTicketCount}
                  </span>
                ) : null}
              </div>
              <div className="mt-4 min-h-0 flex-1">
                {ticketsLoading ? (
                  <ul className="divide-y divide-[#E8E2D0]" aria-busy>
                    {[0, 1, 2].map((i) => (
                      <PortalDashboardTicketRowSkeleton key={i} />
                    ))}
                  </ul>
                ) : displayedTickets.length === 0 ? (
                  <p className="py-6 text-center text-sm text-[#8C8A78]">Ingen aktive sager</p>
                ) : (
                    ...
                    {displayedTickets.map((ticket) => (
                      <li key={ticket.id} className="py-3 first:pt-0">
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          <p className="truncate text-sm font-semibold text-[#2C3020]">{ticket.title}</p>
                        </div>
                        <div className="mt-1.5 flex flex-wrap items-center gap-2">
                          <p className="text-xs text-[#8C8A78]">
                            {ticket.created_at
              ...
              <div className="mt-auto space-y-3 pt-5">
                <Link
                  ...
                  className="inline-flex min-h-11 w-full items-center justify-center rounded-full bg-[#8B9E6B] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-[#7A8A5A]"
                >
                  Opret IT-sag
                </Link>
          ...
          {!preview ? (
            <div className="grid grid-cols-1 gap-6 lg:col-span-3 lg:grid-cols-2">
              <article data-tour="dashboard-latest-report" className={dashboardCardClass}>
                <h2 className="text-sm font-semibold text-[#2C3020]">Seneste rapport</h2>
              <div className="mt-4 min-h-0 flex-1">
                {reportsLoading ? (
                  <div className="space-y-2 py-2" aria-hidden>
                    <div className="h-4 w-3/4 max-w-xs animate-pulse rounded bg-[#EEF2E6]" />
                    <div className="h-3 w-1/2 max-w-[10rem] animate-pulse rounded bg-[#EEF2E6]/90" />
                  </div>
                ) : latestReport ? (
                  <div className="space-y-2">
                    <p className="text-sm font-semibold text-[#2C3020]">{latestReport.title}</p>
                    <p className="text-xs text-[#8C8A78]">
                  ...
                  <div className="py-4">
                    <p className="text-sm font-medium text-[#5C5A48]">Ingen rapporter endnu</p>
              ...
              <article className={`${dashboardCardClass} min-h-0`}>
                ...
                <p className="mt-0.5 text-xs text-[#8C8A78]">denne måned</p>
                <div className="mt-4 min-h-0 flex-1">
                  {historyLoading ? (
                    <div className="space-y-3" aria-hidden>
                      <div className="h-24 w-full animate-pulse rounded-lg bg-[#EEF2E6]" />
                      <div className="h-4 w-48 animate-pulse rounded bg-[#EEF2E6]" />
                    </div>
                  ) : (
                    <>
                      ...
                      {!hasEnoughHealthData ? (
                        <p className="mt-2 text-center text-xs text-[#8C8A78]">
                ...
                <div className="mt-auto pt-4">
                  {historyLoading ? (
                    <div className="h-4 w-44 animate-pulse rounded bg-[#EEF2E6]" aria-hidden />
                  ) : averageUptimePct !== null ? (
                    <p className="text-sm text-[#5C5A48]">
                      Gennemsnitlig oppetid:{" "}
                      <span className="font-bold text-[#2C3020]">{averageUptimePct}%</span>
                    </p>
                  ) : (
                    <p className="text-sm font-medium text-[#5C5A48]">Ikke nok data endnu</p>
                  )}
                </div>
              </article>
            </div>
        ...
        {showUptimeChart ? (
          <section className="mt-4 overflow-hidden rounded-2xl border border-[#D4C9A8] bg-white shadow-sm md:hidden">
            <button
              type="button"
              onClick={() => setChartOpen((open) => !open)}
              className="flex w-full items-center justify-between gap-3 px-4 py-3.5 text-left transition-colors hover:bg-[#EEF2E6]/60"
              aria-expanded={chartOpen}
            >
              <h2 className="text-sm font-semibold text-[#2C3020]">Oppetid de seneste 30 dage</h2>
              <ChevronDown
                className={`h-4 w-4 shrink-0 text-[#8C8A78] transition-transform duration-200 ${
            ...
            {chartOpen ? (
              <div className="border-t border-[#E8E2D0] px-4 pb-5 pt-1">
      ...
      {detail ? (
        <PortalSlideInPanel open onClose={closeDetail}>
            <div className="flex items-start justify-between gap-3 border-b border-[#D4C9A8] px-5 py-4">
              <h2 className="text-lg font-semibold text-[#2C3020]">{detail.friendly}</h2>
              <button
                type="button"
                onClick={closeDetail}
                className="rounded-full p-1.5 text-[#8C8A78] transition hover:bg-[#EEF2E6] hover:text-[#2C3020]"
                aria-label="Luk"
              >
                <X className="h-5 w-5" />
              </button>
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-5">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-[#8C8A78]">
                        Status
                      </p>
                      <span
                    ...
                    <p className="text-sm leading-relaxed text-[#5C5A48]">{expl}</p>
                    {checkedLabel ? (
                      <p className="text-xs text-[#8C8A78]">{checkedLabel}</p>
                    ) : null}
                    {detailLines.length > 0 ? (
                      <div>
                        ...
                        <ul className="mt-2 space-y-1 text-sm text-[#5C5A48]">
                    ...
                    <div className="border-t border-[#D4C9A8] pt-4">
                      <Link
                        href={supportHref}
                        className="inline-flex w-full items-center justify-center rounded-full bg-[#8B9E6B] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#7A8A5A]"
                      >
