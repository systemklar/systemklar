...
export default function PortalRapportPage() {
  ...
  return (
    <div className="w-full p-6 md:p-8">
      <h1 className="text-2xl font-bold text-[#0A1628]">IT-rapport</h1>
      <p className="mt-2 text-sm text-[#2A4868]">
        Dine månedlige IT-statusrapporter — åbn og læs dem her i portalen.
      </p>

      {loading ? (
        <p className="mt-10 text-sm text-[#2A4868]">Henter rapporter...</p>
      ) : reports.length === 0 ? (
        <p className="mt-10 rounded-2xl border border-[#CBD5E8] bg-white px-5 py-8 text-center text-sm text-[#2A4868] shadow-sm">
          <span className="font-medium text-[#0A1628]">Ingen rapporter endnu.</span>
        ...
        <ul className="mt-8 space-y-4">
          {reports.map((r) => (
            <li
              key={r.id}
              className="flex flex-col gap-3 rounded-2xl border border-[#CBD5E8] bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between"
            >
              <div>
                <p className="text-base font-semibold text-[#0A1628]">{r.title}</p>
                <p className="mt-1 text-sm text-[#2A4868]">{periodLabel(r.period_start, r.period_end)}</p>
              </div>
              <Link
                href={`/portal/rapport/${r.id}`}
                className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[#2952A3] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#1E4490]"
              >
                Se rapport
