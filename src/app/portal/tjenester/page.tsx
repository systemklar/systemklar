...
export default function PortalServicesPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (error) {
      logSupabaseError("[portal/tjenester] list", error);
      setRows([]);
    } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (modalMode === "create") {
      ...
      if (error) {
        logSupabaseError("[portal/tjenester] insert", error);
      ...
      const { error } = await supabase
        ...
        .eq("id", editing.id);
      if (error) {
        logSupabaseError("[portal/tjenester] update", error);
        setFormError(error.message);
        setSaving(false);
        return;
      }
  ...
  const handleDelete = async (r: ServiceRow) => {
    const ok = window.confirm(`Slet tjenesten "${r.name}"?`);
    if (!ok) return;
    setDeletingId(r.id);
    const { error } = await supabase.from("services").delete().eq("id", r.id);
    setDeletingId(null);
    if (error) {
      logSupabaseError("[portal/tjenester] delete", error);
      alert(error.message);
      return;
    }
    void load();
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Administrer dine egne ydelser og priser til tilbudsgeneratoren.
            </p>
          </div>
          <button
            type="button"
