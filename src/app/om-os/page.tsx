    ...
    <MarketingShell>
      ...
      <section className="bg-[#F7F4EF] px-6 py-20 md:py-28">
        <ScrollReveal className="mx-auto max-w-3xl text-center">
          <h1 className="text-4xl font-light leading-[1.15] tracking-tight text-[#1E3448] md:text-5xl">
      ...
      <section className="bg-white px-6 py-20 md:py-28">
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-2 lg:gap-16">
          <ScrollReveal>
            <p className="text-sm font-medium uppercase tracking-wider text-[#4A7FA5]">Vores historie</p>
            <h2 className="mt-3 text-3xl font-light tracking-tight text-[#1E3448]">
              Bygget af frustration over uoverskuelig IT
            </h2>
            <p className="mt-4 text-base leading-relaxed text-[#4A6478]">
          ...
          <ScrollReveal staggerMs={100}>
            <ol className="relative border-l border-[#C8D8E4] pl-8">
              {TIMELINE.map((item, index) => (
                <li key={`${item.year}-${item.title}`} className={index < TIMELINE.length - 1 ? "pb-10" : ""}>
                  <span className="absolute -left-[5px] mt-1.5 h-2.5 w-2.5 rounded-full bg-[#4A7FA5]" />
                  <p className="text-xs font-medium uppercase tracking-wider text-[#4A7FA5]">{item.year}</p>
                  <h3 className="mt-1 text-base font-medium text-[#1E3448]">{item.title}</h3>
                  <p className="mt-1 text-sm leading-relaxed text-[#4A6478]">{item.description}</p>
                </li>
              ))}
            </ol>
          </ScrollReveal>
              ...
              return (
                <ScrollReveal key={value.title} staggerMs={index * 100}>
                  <li className="h-full rounded-2xl border border-[#C8D8E4] bg-white p-8">
                    ...
                    <h3 className="mt-5 text-lg font-medium text-[#1E3448]">{value.title}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-[#4A6478]">{value.description}</p>
                  </li>
                </ScrollReveal>
              );
            })}
      ...
      <section className="bg-white px-6 py-20 md:py-28">
        <div className="mx-auto max-w-lg">
          <ScrollReveal>
            <h2 className="text-center text-3xl font-light tracking-tight text-[#1E3448]">Teamet</h2>
          </ScrollReveal>
          <ScrollReveal staggerMs={100}>
            <article className="mt-12 rounded-2xl border border-[#C8D8E4] bg-white p-8 text-center">
              <div
                className="mx-auto flex h-[120px] w-[120px] items-center justify-center rounded-full border border-[#C8D8E4] bg-[#EAF1F7] text-2xl font-medium text-[#4A7FA5]"
                aria-hidden
              >
                BS
              </div>
              <h3 className="mt-6 text-xl font-medium text-[#1E3448]">Benjamin Sørensen</h3>
              <p className="mt-1 text-sm font-medium text-[#4A7FA5]">Grundlægger</p>
