      ...
      <main>
        ...
        <section className="bg-white py-16 md:py-24">
          <div className="mx-auto max-w-7xl px-6">
              ...
              {plans.map((plan, index) => (
                ...
                >
                  <div className="relative h-full">
                    {plan.highlight ? (
                      <div
                        className="pointer-events-none absolute -inset-0.5 animate-pulse rounded-[18px] bg-[#4A7FA5]/20 blur-sm"
                        aria-hidden
                      />
                    ) : null}
                    <article
                      className={`relative flex h-full flex-col rounded-2xl bg-white p-8 ${
                        plan.highlight
                          ? "border-2 border-[#3A6F95] shadow-xl"
                          : "border border-[#C8D8E4] shadow-sm"
                      }`}
                    >
                      {plan.highlight ? (
                        <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#4A7FA5] px-3 py-1 text-xs font-semibold text-white shadow-sm">
                          Mest populær
                        </span>
                      ) : null}
                      <h3 className="text-xl font-semibold uppercase tracking-wide text-[#1E3448]">{plan.name}</h3>
                      <p
                      ...
                      {yearly ? <p className="mt-1 text-xs text-[#4A6478]">faktureres årligt</p> : null}
                      <p className="mt-3 text-xs text-[#4A6478]">{plan.fits}</p>
                      <ul className="mt-6 space-y-3 text-sm text-[#4A6478]">
            ...
            <p className="mt-8 text-center text-sm text-[#4A6478]">
              Ikke sikker på hvilken plan?{" "}
              <Link href="/#roi-beregner" className="font-medium text-[#4A7FA5] hover:underline">
        ...
        <section className="bg-white py-16 md:py-20">
          <div className="mx-auto max-w-5xl px-6">
            <AnimatedSection direction="up">
              <h2 className="text-center text-2xl font-bold tracking-tight text-[#1E3448] md:text-4xl">
                Prøv uden risiko
              </h2>
              <p className="mx-auto mt-4 max-w-2xl text-center text-base text-[#4A6478]">
            ...
            <div className="mt-10 grid grid-cols-1 gap-6 md:mt-12 md:grid-cols-3">
              {reassurance.map(({ icon: Icon, title, desc }, i) => (
                ...
                >
                  <div className="h-full rounded-2xl border border-[#C8D8E4] bg-[#EAF1F7] p-6">
                    <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[#EAF1F7]">
                      <Icon className="h-6 w-6 text-[#4A7FA5]" />
                    </div>
            ...
            <div className="mt-10 space-y-3">
              {faq.map((item, i) => {
                ...
                return (
                  ...
                  >
                    <div className="overflow-hidden rounded-2xl border border-[#C8D8E4] bg-white">
                      <button
                        onClick={() => setOpenFaq(isOpen ? null : i)}
                        aria-expanded={isOpen}
                        className="flex w-full items-center justify-between gap-4 px-6 py-4 text-left"
                      >
                        <span className="font-semibold text-[#1E3448]">{item.q}</span>
                        <ChevronDown
                          className={`h-5 w-5 shrink-0 text-[#4A7FA5] transition-transform duration-300 ${
                      ...
                      >
                        <div className="overflow-hidden">
                          <p className="px-6 pb-5 text-sm leading-relaxed text-[#4A6478]">{item.a}</p>
                        </div>
                      </div>
                    </div>
                  </AnimatedSection>
        ...
        <section className="bg-white py-16 md:py-20">
          <div className="mx-auto max-w-2xl px-6 text-center">
            <AnimatedSection direction="up">
              <h2 className="text-2xl font-bold tracking-tight text-[#1E3448] md:text-3xl">Vil du se det i praksis?</h2>
              <p className="mx-auto mt-4 max-w-xl text-base text-[#4A6478]">
                Book en gratis snak, så gennemgår vi hvilken plan der passer bedst til jer.
              </p>
              <button
                type="button"
                onClick={() => setModalSubject("Demo")}
                className="mt-8 min-h-[44px] rounded-full bg-[#4A7FA5] px-7 py-3 font-semibold text-white transition-colors hover:bg-[#3A6F95]"
              >
                Book en gratis snak
              </button>
            </AnimatedSection>
