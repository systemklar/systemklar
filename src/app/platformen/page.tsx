...
function BrowserChrome({ path }: { path: string }) {
  return (
    <div className="flex items-center gap-2 border-b border-[#CBD5E8] bg-[#E8EEFC] px-4 py-2.5">
      <div className="flex gap-1.5">
        <div className="h-2.5 w-2.5 rounded-full bg-red-400" />
        <div className="h-2.5 w-2.5 rounded-full bg-amber-400" />
        <div className="h-2.5 w-2.5 rounded-full bg-green-400" />
      </div>
      <div className="flex-1 rounded-full bg-white px-3 py-0.5 text-xs text-[#2A4868]">{path}</div>
    </div>
  );
}
...
function MockupCard({ children }: { children: ReactNode }) {
  return (
    <div className="cursor-default overflow-hidden rounded-2xl border border-[#CBD5E8] bg-white shadow-xl transition-transform duration-500 hover:scale-[1.02]">
      {children}
    </div>
  );
}
...
function Sidebar({ active }: { active: string }) {
  ...
  return (
    <div className="flex w-36 shrink-0 flex-col gap-1 border-r border-[#E4EAF5] bg-[#F2F5FA] p-3">
      <div className="mb-2 px-1 text-[10px] font-bold text-[#2952A3]">systemklar</div>
        ...
        return (
          <div
            ...
            className={`flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs ${
              isActive ? "bg-[#E8EEFC] font-medium text-[#1E4490]" : "text-[#9AAAC8]"
            }`}
          >
...
const dashboardStats: Array<{ value: string; label: string; cls: string }> = [
  ...
  { value: "apr", label: "Rapport", cls: "bg-[#E8EEFC] text-[#1E4490]" },
];
    ...
    <MockupCard>
      ...
      <div className="flex" style={{ minHeight: "320px" }}>
        ...
        <div className="flex-1 bg-white p-5">
          <div className="mb-0.5 text-sm font-bold text-[#0A1628]">Goddag, Møllers VVS</div>
          <div className="mb-4 text-xs text-[#2A4868]">Onsdag den 7. maj 2026</div>
          ...
          <div className="mb-2 text-xs font-semibold text-[#0A1628]">Seneste sager</div>
          {dashboardCases.map((c, i) => (
            <div
              key={c.title}
              className="mb-1.5 flex items-center justify-between rounded-lg bg-[#E8EEFC] px-3 py-2"
              style={{ animation: `mockupFadeIn 0.4s ease-out ${0.3 + i * 0.1}s both` }}
            >
              <span className="text-xs text-[#2A4868]">{c.title}</span>
  ...
  return (
    <MockupCard>
      ...
      <div className="bg-white" style={{ minHeight: "320px" }}>
        <div className="border-b border-[#E4EAF5] px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="text-sm font-bold text-[#0A1628]">Sag #1284 – Printer virker ikke</div>
            <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-700">
              Aktiv
            </span>
          </div>
          <div className="mt-1 text-[10px] text-[#2A4868]">Oprettet for 12 minutter siden · 2 svar</div>
        </div>
          ...
          {supportMessages.map((m, i) => (
            <div
              ...
              className={`max-w-xs rounded-2xl px-3 py-2 text-xs ${
                m.from === "user"
                  ? "self-end rounded-tr-sm bg-[#2952A3] text-white"
                  : "self-start rounded-tl-sm bg-[#E8EEFC] text-[#2A4868]"
              }`}
        ...
        <div className="border-t border-[#E4EAF5] p-3">
          <div className="flex items-center gap-2 rounded-full border border-[#CBD5E8] bg-[#F8FCFF] px-3 py-1.5">
            <Paperclip className="h-3.5 w-3.5 text-[#2A4868]" />
            <span className="flex-1 text-xs text-[#2A4868]">Skriv en besked …</span>
            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#2952A3]">
...
function ReportMockup() {
  const stats: Array<{ value: string; label: string; cls: string }> = [
    ...
    { value: "3", label: "Løste sager", cls: "bg-[#E8EEFC] text-[#1E4490]" },
    { value: "0", label: "Åbne sager", cls: "bg-[#E8EEFC] text-[#1E4490]" },
  ];
  return (
    <MockupCard>
      ...
      <div className="bg-white p-5" style={{ minHeight: "320px" }}>
        <div className="flex items-start justify-between">
          <div>
            <div className="text-sm font-bold text-[#0A1628]">IT-rapport – april 2026</div>
            <div className="mt-0.5 text-xs text-[#2A4868]">Møllers VVS · Genereret 1. maj</div>
          </div>
          <span className="rounded-full bg-[#E8EEFC] px-2 py-0.5 text-[10px] font-medium text-[#1E4490]">PDF klar</span>
        </div>
        ...
        <div className="mt-4 mb-1.5 text-xs font-semibold text-[#0A1628]">Anbefaling til næste måned</div>
        <div
        ...
        <div className="mt-3 flex items-center justify-between text-[10px] text-[#2A4868]">
    ...
    <MockupCard>
      ...
      <div className="bg-white p-5" style={{ minHeight: "320px" }}>
        <div className="mb-3 flex items-center justify-between">
          <div className="text-sm font-bold text-[#0A1628]">Kodebank</div>
          <span className="inline-flex items-center gap-1 rounded-full bg-green-50 px-2 py-0.5 text-[10px] font-medium text-green-700">
            <Lock className="h-3 w-3" />
            Krypteret
          </span>
        </div>
        <div className="mb-3 flex items-center gap-2 rounded-lg border border-[#CBD5E8] bg-[#F8FCFF] px-3 py-2">
          <Search className="h-3.5 w-3.5 text-[#2A4868]" />
          <span className="flex-1 text-xs text-[#2A4868]">Søg på navn, kategori eller URL …</span>
        </div>
        <div className="space-y-2">
          {vaultEntries.map((entry, i) => (
            <div
              key={entry.name}
              className="flex items-center justify-between rounded-lg bg-[#E8EEFC] px-3 py-2"
              style={{ animation: `mockupFadeIn 0.4s ease-out ${i * 0.08}s both` }}
            >
              <div>
                <div className="text-xs font-semibold text-[#0A1628]">{entry.name}</div>
                <div className="mt-0.5 inline-flex items-center gap-1 text-[10px] text-[#2A4868]">
                  <Tag className="h-2.5 w-2.5" />
                  {entry.category}
                </div>
              </div>
              <span className="text-xs tracking-wider text-[#2A4868]">••••••••</span>
            </div>
          ))}
        </div>
      </div>
...
function SystemsMockup() {
  return (
    <MockupCard>
      ...
      <div className="bg-white p-5" style={{ minHeight: "320px" }}>
        <div className="mb-3 flex items-center justify-between">
          <div className="text-sm font-bold text-[#0A1628]">Systemer</div>
          <span className="inline-flex items-center gap-1 rounded-full bg-[#E8EEFC] px-2 py-0.5 text-[10px] font-medium text-[#1E4490]">
        ...
        <div className="space-y-2">
          {systems.map((sys, i) => {
            ...
            return (
              <div
                key={sys.name}
                className="flex items-center justify-between rounded-lg border border-[#CBD5E8] bg-[#F8FCFF] px-3 py-2.5"
                style={{ animation: `mockupFadeIn 0.4s ease-out ${i * 0.1}s both` }}
              >
                <div className="flex items-center gap-2.5">
                  <span className={`h-2 w-2 rounded-full ${cfg.dot}`} aria-hidden />
                  <div>
                    <div className="text-xs font-semibold text-[#0A1628]">{sys.name}</div>
                    <div className="text-[10px] text-[#2A4868]">{sys.type}</div>
                  </div>
                </div>
        ...
        <div className="mt-3 flex items-center gap-1.5 text-[10px] text-[#2A4868]">
...
const teamMembers: Array<{ name: string; role: string; initials: string; cls: string }> = [
  { name: "Benjamin Sørensen", role: "Administrator", initials: "BS", cls: "bg-[#2952A3] text-white" },
  { name: "Maria Larsen", role: "Medlem", initials: "ML", cls: "bg-[#E8EEFC] text-[#1E4490]" },
  { name: "Jens Kristensen", role: "Medlem", initials: "JK", cls: "bg-[#E8EEFC] text-[#1E4490]" },
];
...
function TeamMockup() {
  return (
    <MockupCard>
      ...
      <div className="bg-white p-5" style={{ minHeight: "320px" }}>
        <div className="mb-3 flex items-center justify-between">
          <div className="text-sm font-bold text-[#0A1628]">Team</div>
          <span className="inline-flex items-center gap-1 rounded-full bg-[#E8EEFC] px-2 py-0.5 text-[10px] font-medium text-[#1E4490]">
          ...
          {teamMembers.map((m, i) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-xs font-semibold text-[#0A1628]">{m.name}</div>
                  <div className="text-[10px] text-[#2A4868]">{m.role}</div>
                </div>
              </div>
              <ChevronRight className="h-3.5 w-3.5 text-[#2A4868]" />
            </div>
          ))}
        </div>
        <button
          className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-[#CBD5E8] px-3 py-2 text-xs font-medium text-[#2952A3]"
...
function FeatureCopy({
  ...
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-widest text-[#2952A3]">{label}</p>
      <h2 className="mt-3 text-2xl font-bold tracking-tight text-[#0A1628] md:text-4xl">{title}</h2>
      <p className="mt-4 text-base leading-relaxed text-[#2A4868]">{description}</p>
      <ul className="mt-6 space-y-3">
        {points.map((p) => (
          <li key={p} className="flex items-start gap-2 text-sm text-[#0A1628]">
            <CheckCircle className="mt-0.5 h-4 w-4 shrink-0 text-[#2952A3]" />
            <span>{p}</span>
          </li>
        ))}
      </ul>
  ...
  return (
    <MarketingShell>
      <main>
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-[#2952A3] to-[#0A1628] py-20 pt-32 md:py-32 md:pt-40">
          <div
