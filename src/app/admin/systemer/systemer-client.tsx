...
function monitoringStatusStyles(status: AdminSystemReference["monitoringStatus"]) {
  ...
  if (status === "pending") {
    return {
      dot: "bg-[#9AAAC8]",
      badge: "bg-[#EEF2FA] text-[#2A4868] ring-[#CBD5E8]/80",
    };
  }
  return {
    dot: "bg-[#E05040]",
...
function ExternalHref({
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-1 rounded-full border border-[#CBD5E8] px-2.5 py-1 text-xs font-semibold text-[#2952A3] transition hover:bg-[#E8EEFC]"
    >
      {label}
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <aside
        ...
        className="relative flex h-full w-full max-w-lg flex-col overflow-hidden border-l border-[#CBD5E8] bg-white shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-3 border-b border-[#CBD5E8] px-5 py-4">
          <div className="flex min-w-0 items-start gap-3">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#E8EEFC] text-[#2952A3]">
              <Icon className="h-5 w-5" aria-hidden />
            </span>
            <div className="min-w-0">
              <h2 id="system-detail-title" className="text-lg font-bold text-[#0A1628]">
                {system.name}
              </h2>
              <p className="mt-0.5 text-sm text-[#2A4868]">{reference.categoryShortLabel}</p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-[#2A4868] hover:bg-[#E8EEFC]"
            aria-label="Luk"
          >
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex rounded-full bg-[#E8EEFC] px-2.5 py-0.5 text-xs font-semibold text-[#0A1628]">
              {reference.categoryShortLabel}
            </span>
            <span className="inline-flex rounded-full bg-[#E8EEFC] px-2.5 py-0.5 text-xs font-semibold text-[#2952A3]">
          ...
          <p className="mt-4 text-sm leading-relaxed text-[#2A4868]">{reference.description}</p>

          <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div>
              <dt className="text-xs font-medium text-[#2A4868]">Integration</dt>
              <dd className="mt-0.5 font-medium text-[#0A1628]">
                {integrationTypeLabel(reference.integrationType)}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-[#2A4868]">Sværhedsgrad</dt>
              <dd className="mt-0.5 font-medium capitalize text-[#0A1628]">
          ...
          <section className="mt-6">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-[#2A4868]">
              Opsætningsnoter (intern)
            </h3>
            <ul className="mt-2 list-inside list-disc space-y-1 text-sm text-[#2A4868]">
          ...
          <section className="mt-6">
            ...
            {customers.length === 0 ? (
              <p className="mt-2 text-sm text-[#2A4868]">Ingen kunder har valgt dette system endnu.</p>
            ) : (
              <ul className="mt-2 divide-y divide-[#E4EAF5] rounded-xl border border-[#CBD5E8]">
                {customers.map((c) => (
                  <li key={c.organisationId}>
                    <Link
                      href={`/admin/customers/${c.organisationId}`}
                      className="block px-3 py-2.5 text-sm font-medium text-[#2952A3] hover:bg-[#E8EEFC]"
                    >
                      {c.organisationName}
                    </Link>
                  </li>
...
function SystemCard({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className="flex w-full flex-col rounded-2xl border border-[#CBD5E8] bg-white p-5 text-left shadow-sm transition hover:border-[#CBD5E8] hover:shadow-md"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-bold text-[#0A1628]">{system.name}</p>
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            <span className="inline-flex rounded-full bg-[#E8EEFC] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[#2A4868]">
              {reference.categoryShortLabel}
            </span>
            <span className="inline-flex rounded-full bg-[#E8EEFC] px-2 py-0.5 text-[10px] font-semibold text-[#2952A3]">
              {integrationTypeLabel(reference.integrationType)}
            </span>
          </div>
        </div>
...
export default function AdminSystemerClient() {
  ...
  return (
    <div>
      <div className="border-b border-[#CBD5E8] pb-6">
        <p className="mb-1 text-xs text-[#2A4868]">Admin</p>
        <h1 className="text-2xl font-bold text-[#0A1628]">Systemreference</h1>
        <p className="mt-2 max-w-2xl text-sm text-[#2A4868]">
      ...
      <div className="mt-6 space-y-4">
        <label className="relative block">
          <Search
            className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#2A4868]"
            aria-hidden
          />
          <input
            type="search"
            ...
            className="w-full rounded-xl border border-[#CBD5E8] bg-white py-3 pl-10 pr-4 text-sm text-[#0A1628] outline-none transition focus:border-[#2952A3] focus:ring-2 focus:ring-[#2952A3]/20"
          />
        </label>

        <nav className="-mb-px flex flex-wrap gap-1 border-b border-[#CBD5E8] pb-px" aria-label="Kategorier">
            ...
            return (
              <button
                ...
                className={`rounded-t-lg px-3 py-2 text-sm font-semibold transition ${
                  active
                    ? "border-b-2 border-[#2952A3] text-[#2952A3]"
                    : "text-[#2A4868] hover:bg-[#E8EEFC] hover:text-[#0A1628]"
                }`}
              >
                {tab}
              </button>
      ...
      {loading ? (
        <p className="mt-8 flex items-center gap-2 text-sm text-[#2A4868]">
        ...
        <p className="mt-8 text-sm text-[#2A4868]">Ingen systemer matcher din søgning.</p>
      ) : (
