...
export default function AdminCustomerPortalDashboardPreview() {
  ...
  if (loading) {
    return <p className="text-sm text-slate-600">Indlæser dashboard...</p>;
  }

  if (error || !org) {
    return (
      <div className="space-y-4">
        <Link href={`/admin/customers/${id || ""}`} className="text-sm font-semibold text-sky-700 hover:underline">
          ← Tilbage til kunde
        </Link>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <Link href={`/admin/customers/${org.id}`} className="text-sm font-semibold text-sky-700 hover:underline">
          ← Tilbage til kunde
        </Link>
      </div>
